...
type Validator struct {
	...
	ValidateUsernameAndHostname     func(username_hostname string) []error
...
func NewValidator() *Validator {
	...
	valid_IPAddress_characters := NewIPAddressWhitelist()
	...
	x := Validator{
		...
		ValidateIPAddress: func(IPAddress string) []error {
			return valid_IPAddress_characters.ValidateIPAddress(IPAddress)
		},
		ValidateDomainName: func(domain_name string) []error {
		...
		ValidateUsernameAndHostname: func(username_hostname string) []error {
			...
			if !strings.Contains(username_hostname, "@") {
				errors = append(errors, fmt.Errorf("username does not contain the @ symbol"))
				return errors
			}

			parts := strings.Split(username_hostname, "@")

			if len(parts) > 2 {
				errors = append(errors, fmt.Errorf("username contains more than one @ symbol"))
			}

			username := parts[0]
			username_errors := valid_username_characters.ValidateUsername(username)
			...
			hostname := parts[1]
			hostname_errors := valid_IPAddress_characters.ValidateIPAddress(hostname)
			if hostname_errors != nil {
